...
from app.utils import atr_bucket, interval_to_minutes, liquidity_bucket, rsi_bucket, score_bucket, volume_ratio_bucket
...
def compute_bot_like_score(frame: pd.DataFrame) -> pd.Series:
    ...
    upside_distance = np.maximum(rsi - 70.0, 0.0)
    downside_distance = np.maximum(30.0 - rsi, 0.0)
    threshold_distance = np.maximum(upside_distance, downside_distance)
    bull_trend = (close > ema20) & (ema20 > ema50)
    bear_trend = (close < ema20) & (ema20 < ema50)
    trend_score = np.where(bull_trend | bear_trend, 12.0, 5.0)
    stretch = np.maximum(np.abs(close - ema20) / close.replace(0, np.nan), 0.0).fillna(0.0)
...
def enrich_indicators(frame: pd.DataFrame, *, interval: str, rsi_length: int = 14) -> pd.DataFrame:
    enriched = frame.copy()
    ...
    enriched["range_pct"] = ((enriched["high"] - enriched["low"]) / enriched["open"] * 100.0).replace([np.inf, -np.inf], np.nan)
    ...
    enriched["close_position_in_range"] = ((enriched["close"] - enriched["low"]) / price_range).clip(lower=0.0, upper=1.0)
    enriched["score"] = compute_bot_like_score(enriched)
    enriched["direction"] = np.where(enriched["rsi"] <= 30.0, "oversold", np.where(enriched["rsi"] >= 70.0, "overbought", "neutral"))
    enriched["rsi_bucket"] = enriched["rsi"].apply(rsi_bucket)
    enriched["atr_bucket"] = enriched["atr_pct"].apply(atr_bucket)
    enriched["volume_bucket"] = enriched["volume_ratio"].apply(volume_ratio_bucket)
    enriched["liquidity_bucket"] = enriched["rolling_quote_volume_24h"].apply(liquidity_bucket)
    enriched["score_bucket"] = enriched["score"].apply(score_bucket)
    return enriched
